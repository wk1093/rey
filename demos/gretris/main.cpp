...
int CRISS_CROSS_GUY_BLOCK_I_DUNNO_THE_NAME[20] = {
	// 4 blocks (on a 0-3 grid)
	1, 0,
	2, 0,
	0, 1,
...
int OTHER_CRISS_CROSS_GUY_BLOCK_THAT_I_DUNNO_THE_NAME_OF_AS_WELL[20] = {
	// 4 blocks (on a 0-3 grid)
	0, 0,
	1, 0,
	1, 1,
		...
		if (shouldReset) {
			...
			if (shouldResetTimer > 1.0f / fallSpeed) {
				...
				case 4:
					...
					break;
				case 5:
					for (int i = 0; i < 20; i++) { currentBlock[i] = CRISS_CROSS_GUY_BLOCK_I_DUNNO_THE_NAME[i]; }
					break;
				case 6:
					for (int i = 0; i < 20; i++) { currentBlock[i] = T_BLOCK[i]; }
					break;
				case 7:
					for (int i = 0; i < 20; i++) { currentBlock[i] = OTHER_CRISS_CROSS_GUY_BLOCK_THAT_I_DUNNO_THE_NAME_OF_AS_WELL[i]; }
					break;
				}
				shouldReset = false;
